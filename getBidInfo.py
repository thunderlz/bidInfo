...
def Main():
    ...
    url='https://www.chinabidding.cn/search/searchzbw/search2?keywords=招标代理服务&areaid=20&categoryid=&b_date=month'
    headers = {
    ...
    with open('bid.html','at') as f:

        tb=getChinabiddingHtml(url,headers).find('table',{'id':'cTable'})
        print(tb,file=f)
        print(tb.find('a',text='后一页'))
        while tb.find('a',text='后一页'):
            link=tb.find('a',text='后一页')['href']
            print(link)
            tb=getChinabiddingHtml(head+link,headers=headers)
            print(tb,file=f)
    return
